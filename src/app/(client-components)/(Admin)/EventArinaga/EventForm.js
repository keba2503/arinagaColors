...
const EventForm = ({ event }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (image) {
      ...
      formData.append('folder', 'arinagacolors/eventArinaga');
        ...
        if (oldImagePublicId) {
          ...
          try {
            await axios.delete('/api/cloudinaryEventArinag', {
              ...
              data: {
                public_id: `arinagacolors/eventArinaga/${oldImagePublicId}`,
              },
            });
          } catch (error) {
    ...
    try {
      const response = await fetch(
        `/api/eventArinaga/${event ? event.id : ''}`,
        {
