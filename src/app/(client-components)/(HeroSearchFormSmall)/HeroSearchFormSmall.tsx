...
const HeroSearchFormSmall: FC<HeroSearchFormSmallProps> = ({className = "", defaultTab = "Stays", onTabChange, defaultFieldFocus}) => {
            ...
            <ul className="h-[88px] flex justify-center space-x-5 sm:space-x-9">
                {TABS.map((tab) => {
                    ...
                    return (
                        <li
                            onClick={() => {
                                setTabActive(tab);                                onTabChange && onTabChange(tab);
                            }}
